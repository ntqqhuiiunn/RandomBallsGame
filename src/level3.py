...
class Level3(OriginalMode):
    ...
    def _setUpColor(self):
        ...
        self.wrongFruit = random.choice(list(self.colorDict.keys()))
        self.wrongColor = random.choice(list(self.colorDict.values()))
    ...
    def addWrongText(self, frame, check):
        otherFrame = Image.fromarray(frame)
        draw = ImageDraw.Draw(otherFrame)
        if len(self.wrongFruit) <= 6:
            position = (825, 270)
        else:
            position = (785, 270)
        draw.text(position, self.wrongFruit,
                  fill=self.colorCode[self.wrongColor], font=self.font)
        if check:
            self.wrongFruit = random.choice(list(self.colorDict.keys()))
            self.wrongColor = random.choice(list(self.colorDict.values()))
        return numpy.array(otherFrame)
        ...
        while True:
            ...
            if not check and not checkLose:
                if retStart:
                    ...
                    mask = self._setUpSide(mask, level=3)
                    ...
                    if len(hand) > 0:
                        if hand[0]["type"] == "Right":
                            ...
                            if index >= 0:
                                ...
                                selectedName = self.fruits[categoryIndex]
                                selectedColor = self.colorDict[selectedName]
                                if selectedName == self.wrongFruit or selectedColor == self.wrongColor:
                                    brokenBomb += 5
                                    changeWrongTarget = True
                    ...
                    mask = self.addWrongText(mask, changeWrongTarget)
                ...
                if retEnd:
                    ...
                    self.buttons.setupQuit(mask, reversed=False)
                    showCamera = False
                    hand, image = self.detector.findHands(frame, draw=True)
                    if len(hand) > 0:
                        landmarks = hand[0]['lmList']
...
if __name__ == "__main__":
    ...
    game = Level3(config, video, detector)
    game.run()
